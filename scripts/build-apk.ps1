...
& "$bt\aapt2.exe" link -o "$build\base.apk" --manifest "$build\AndroidManifest.xml" -I $platform --java "$build\generated" --min-sdk-version 29 --target-sdk-version 36 --version-code 13 --version-name 1.4.1 --proguard "$build\resources.pro" "$build\resources.zip"
Check 'resource link'
...
$apk = Join-Path $root 'dist\MimiAutoComboLauncher-1.4.1.apk'
